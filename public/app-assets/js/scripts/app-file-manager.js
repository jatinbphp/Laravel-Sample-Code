...
$(function () {
    ...
    $('body').on('click', '.app-file-info .fonticon', function () {
        sideBarInfo.addClass('show');
        appContentOverlay.addClass('show');

        let info = $(this).data('json');
        $('#infoDate').html(info.date);
        $('#infoSize').html(info.size);
        $('#infoSize2').html(info.size);
        if (info.type.hasOwnProperty('type')) {
            let url = sideBarInfo.data('download') + '?file=' + info.name + '&location=' + info.location;
            let image = info.type.icon;
            if (info.type.type === 'Image File') {
                image = info.url;
            }   $('#infoType').html(info.type.type);

            $('#infoIcon').html('<img src="' + image + '"   class="mt-5 mb-5 responsive-img ">');
            $('#infoDowload').html('<a href="' + url + '" target="_blank">Download</a>');
        } else {
            $('#infoIcon').html('FOLDER');
            $('#infoType').html(info.type);
            $('#infoDowload').html('-');
        }
        $('#infoLocation').html(info.location);
        $('#infoName').html(info.name);
        $('.app-file-action-icons .delete').data('id', info.id);
    });
