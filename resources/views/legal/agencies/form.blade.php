<form id="form-add-salary-loan" name="form-add-salary-loan" class="display-flex flex-column h-100 justify-content-between" enctype="multipart/form-data">
  <div class="row">
    <div class="input-field col s6 k-dropdown">
      <label for="name3">
        {{trans('message.user')}} <span>*</span>
      </label>
      <select id="user_id" name="user_id" class="userSelect">
          ...
          <optgroup label="{{$role}}">
            @foreach(getAllUserByRole($roleId) as $userId => $user)
              <option value="{{$userId}}" data-badge="" @if(isset($salaryLoan) && $userId == $salaryLoan->user_id) selected='selected' @endif>{{$user}}</option>
            @endforeach
          </optgroup>
          @endforeach
      </select>
  ...
  <div class="row">
    <div class="input-field col s12">
      <div class="modal_button_center">
        @if(isset($salaryLoan))
          <button class="ml-4  btn k-button-fill k-btn  submit-edit-interview action-btn updateAdminSalaryLoan" name="action" type="submit" data-id="{{$salaryLoan->id}}">
            <img src="{{asset('img/edit.png')}}" alt="">
            {{trans('message.update')}} {{trans('message.salary')}} {{trans('message.loan')}}
          </button>
        @else
          <button class="action-btn  btn k-button-fill k-icon submitAdminSalaryLoan" type="submit">
