<form method="POST" id="form-add-staff-question" name="form-add-staff-question">
		...
		<div class="input-field col s12 k-dropdown">
			...
			<select name="role_id" id="role_id"   class="browser-default">
				...
				@foreach(array_diff(getRoles(),['Administrator','Model']) as $roleId => $role)
					<option value="{{$roleId}}" @if($staffQuestion->role_id == $roleId) selected='selected' @endif>{{$role}}</option>
				@endforeach
			</select>
	...
	<div class="row">
		<div class="input-field col s12 k-input-text">
			...
			<input type="text" name="question" id="question" class="validate k-txt-box" value="{{$staffQuestion->question}}" />
			<span class="k-error error" id="question_error"></span>
		</div>
	</div>

	<div class="modal_button_center">
		<button class="action-btn  btn k-button-fill k-icon updateStaffQuestion right mb-3" type="button" data-id="{{$staffQuestion->id}}"><img src="{{asset('img/plus-bl.png')}}">{{trans('message.update')}} {{trans('message.staff')}} {{trans('message.question')}}</button>
	</div>
</form>
